...
class InitCommandTest < Minitest::Test
  ...
  test "displays help" do
    ...
    assert_stdout_includes "Usage: i18n-json init [options]"
  end

  test "initializes project" do
    cli = I18nJSON::CLI.new(
      argv: %w[init --config test/output/i18n-json.yml],
      stdout: stdout,
      stderr: stderr
    )

    assert_exit_code(0) { cli.call }
    assert_file "test/output/i18n-json.yml"
    assert_equal "", stdout_text
  end

  test "rejects existing config file" do
    config_file = "test/output/i18n-json.yml"
